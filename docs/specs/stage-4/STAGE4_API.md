...
## 6.1a. Куда можно поставить помощника

```
GET /api/helpers/objects
→ 200 { items: [{ id, name, requiredProfessionCode, requiredProfessionLevel,
                  ownerType, workerSlots }] }
```

Только объекты хозяина и его бригады (решение заказчика по В10) и только
переделы не выше потолка помощника. Список считает сервер: правило живёт в
одном месте, иначе экран предлагает объект, на котором мутация откажет.
...
409 HELP_006  помощник отработал суточную норму: 6 смен
409 HELP_007  объект не ваш и не бригадный
...
## 6.4. Закрытие смены помощника

```
POST /api/helpers/:id/claim  Idempotency-Key
→ 200 { shiftId, salary, newBalance, helper: { professionExp, professionLevel } }
```

Ручка отдельная, вопреки первой редакции этого документа. Через
`POST /api/work/shifts/:id/claim` смену помощника закрыть нельзя (`WORK_008`):
там опыт идёт в `CharacterProfession` хозяина и зарплата считается без
множителя 0.6, то есть общая ручка выдавала бы за помощника полную ставку и
чужой опыт.
