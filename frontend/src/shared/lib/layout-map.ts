...
/**
 * Высота игровой части главного экрана — сцена без полосы чата.
 *
 * Чат с онлайном занимает нижнюю пятую часть холста, а транспорта под
 * него нет до Этапа 3: лента демонстрационная и видна только автору.
 * Отдавать 195 из 900 точек холста под декорацию дорого — сцена
 * кадрируется по этой высоте, и всё остальное растёт во столько же раз,
 * во сколько уменьшился холст (до +28% там, где масштаб упирался в высоту).
 *
 * 705 — не на глаз: верхняя кромка нарисованной рамки чата на
 * menu-plate.webp лежит на строках 705–707 (картинка ровно 1550x900,
 * то есть один к одному с координатами сцены).
 */
export const MENU_GAME_H = 705
...
export const MENU = {
  ...
  /** Наличные персонажа в шапке. Ячейка плашки между надорванным
 *    разделителем (он нарисован на 1270…1284) и «выходом» (левый край
 *    1471,5) в макете пуста — сумма встаёт туда и прижимается вправо.
 *    Ширина 150 — это всё свободное место до разделителя; длиннее сумма
 *    не растёт, а ужимается, поэтому на разделитель не наедет никогда.
 *    Высоты нет намеренно: строка садится по той же верхней кромке и с тем
 *    же сдвигом, что остальные пункты, иначе уезжает ниже нарисованной
 *    полосы — бумага в макете кончается около 26. */
  navMoney: { x: 1286, y: 5.5, w: 150 },
  navFontSize: 21.9,
  navDy: -2,
  /** Область выреза шапки: полоса верхнего меню целиком, от левого края
      сцены до правого. Низ снят по нарисованной кромке плашки в макете
      «Фон основного мнею Магазин» (холст 0…68 -> сцена 0…34): ниже уже
      начинается небо. Нужна боевому экрану — он берёт ту же шапку
      вырезом из той же подложки, как мобильная шторка берёт карточку. */
  navStrip: box(0, 0, 1550, 34),
  ...
  districtGap: 8,
  /** Районы города — те шесть, что нарисованы в макете «Фон основного
      мнею Магазин». Седьмого, «Фермы и колхозы», в макете нет; он не
      описан и в ТЗ, где на карту просили добавить «Бары» и «Бригаду».
      Пришёл из коммита про иерархию локаций рынка и утащил за собой
      спецслучай /work?from=agriculture в districtKey.

      Ферма и растения уехали в Промзону: ТЗ Этапа 3 само требует, чтобы
      «завод, колхоз и бар использовали один цикл», значит разводить
      аграрное производство в отдельный район незачем.

      Адрес у всех одного вида — /district/<ключ>, кроме Центра: он же
      главный экран. */
  districts: [
  ...
  rooms: {
    ...
    station: [
      // «Склад» был описан в SOON с самого начала, но в навигацию не
      // выведен: попасть в него было нельзя ниоткуда. Вокзалу он к месту —
      // в сиде там стоит объект «Склад у вокзала».
  ...
  card: {
    ...
    /** Ширина 78 снята с макета, где ник нарисован коротким — в неё влезает
        знаков шесть. Настоящие ники длиннее, и остаток добирался сжатием:
        16 знаков ужимались втрое. Поле продлено вправо по свободной бумаге
        (правый край листа 340.5), высота строки под ним свободна до слотов
        (424) и зон тела (407). Короткие ники стоят там же, где стояли. */
    nickname: { x: 132.5, y: 377.5, w: 200, size: 22.7 },
  ...
  /** Полоса чата целиком — то, что вырезается из подложки, когда чат
      разворачивают. Верх по нарисованной кромке рамки (705), низ — край
      холста: ниже 892 идёт только поле ввода и его отступ. */
  chatPanel: box(0, MENU_GAME_H, 1550, 900 - MENU_GAME_H),
