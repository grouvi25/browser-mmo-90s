...
import { type BattleAction, type BattleTurn } from '../../shared/types/api.types'
...
function buildLogLines(result: RoundResult, charId: string | null): LogEntry[] {
  ...
  if (result.battleOver) {
    ...
    lines.push({
      text: won
        ? `=== ПОБЕДА! Опыт: +${result.expGain} | Навык: +${result.weaponExpGain?.toFixed(1)} | Деньги: +${result.moneyReward} ===`
        : '=== ПОРАЖЕНИЕ ===',
      type: 'result',
    })
    if (result.newLevel && result.newLevel > 1) {
...
export function BattlePage() {
  ...
  const actionMut = useMutation({
    ...
    onSuccess: (data: RoundResult) => {
      const newLines = buildLogLines(data, char?.id ?? null)
  ...
  if (isLoading) return <div className="loading"><span className="spinner" />Загрузка боя...</div>
  if (!battleData && !battleId) return <div className="alert alert-error">Бой не найден</div>

  const liveState = battleData?.liveState
  const battle    = battleData?.battle

  // Find player & enemy participants from live state
  ...
  const enemyHpPct  = (enemyHp / enemyHpMax) * 100

  const isMyTurn = !battleFinished && !actionMut.isPending
  const currentRound = liveState?.roundNumber ?? 1
      ...
      <div className="panel">
        <div className="panel-header">
          ...
          <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>
            Раунд {currentRound} | {battleFinished ? '— ЗАВЕРШЁН —' : 'Идёт...'}
          </span>
        </div>
        <div className="panel-body">
          ...
          {battleFinished && finishResult && (
            <div className="panel mt12" style={{ borderColor: finishResult.result === 'PVE_WIN' ? 'var(--success)' : 'var(--danger)' }}>
              <div className="panel-header" style={{ borderColor: 'inherit' }}>
              ...
              <div className="panel-body">
                <table className="data-table">
                  <tbody>
                    {finishResult.expGain != null && (
                      <tr><td>Боевой опыт</td><td style={{ color: 'var(--xp)' }}>+{finishResult.expGain}</td></tr>
                    )}
                    {finishResult.weaponExpGain != null && (
                      <tr><td>Навык оружия</td><td style={{ color: 'var(--accent)' }}>+{Number(finishResult.weaponExpGain).toFixed(2)}</td></tr>
                    )}
                    {finishResult.moneyReward != null && finishResult.moneyReward > 0 && (
                      <tr><td>Деньги</td><td className="money">+{finishResult.moneyReward}</td></tr>
                    )}
                    {finishResult.newLevel != null && finishResult.newLevel > 1 && (
                      <tr><td>Уровень</td><td style={{ color: 'var(--gold)' }}>🎉 {finishResult.newLevel}</td></tr>
                    )}
                  </tbody>
                </table>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                  ...
                  <button className="btn btn-danger"
                    onClick={() => {
                      ...
                      setLog([{ text: 'Новый бой!', type: 'system' }])
                      navigate('/profile')
                    }}
                  >
                    ⚔️ Ещё раз
