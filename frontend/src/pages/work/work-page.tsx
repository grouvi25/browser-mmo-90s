...
const STATUS:Record<string,string>={ACTIVE:'идёт',READY:'смена отработана',CLAIMED:'оплачено',CANCELLED:'брошена'}
...
export function WorkPage(){
 ...
 return <div>
  <div className="panel"><div className="panel-header"><span className="panel-title">Рабочая смена</span><span>{current.data?.daily.shiftsUsedToday??0} / {current.data?.daily.shiftsLimit??8} за сутки</span></div>
   <div className="panel-body">{shift?<div><b>{shift.productionObject?.name??'Объект'}</b><div>Статус: {STATUS[shift.status]??shift.status}</div><div style={{fontFamily:'var(--font-mono)',fontSize:24}}>{remaining>0?time(remaining):'ГОТОВО'}</div>
    <button className="btn btn-success" disabled={remaining>0||claim.isPending} onClick={()=>claim.mutate(shift.id)}>Забрать</button>{' '}
    <button className="btn" disabled={shift.status!=='ACTIVE'||cancel.isPending} onClick={()=>cancel.mutate(shift.id)}>Бросить</button></div>:<span className="text-dim">Смена не начата.</span>}</div>
  </div>
  <div className="panel"><div className="panel-header"><span className="panel-title">Объекты города</span></div><div className="panel-body"><table className="data-table"><thead><tr><th>Объект</th><th>ПУ</th><th>Время</th><th>Оплата</th><th>Выработка</th><th/></tr></thead><tbody>{objects.data?.items.map(o=><tr key={o.id}><td>{o.name}</td><td>{o.requiredProductionLevel}</td><td>{o.shiftDurationMinutes} мин</td><td>{o.baseSalary}</td><td>{o.producesResourceCode?`${o.outputAmountMin}-${o.outputAmountMax} ${o.producesResourceCode}`:'-'}</td><td><button className="btn btn-sm btn-primary" disabled={o.locked||!!shift||start.isPending} onClick={()=>start.mutate(o.id)}>{o.locked?'Закрыт':'Выйти'}</button></td></tr>)}</tbody></table></div></div>
 </div>
}
