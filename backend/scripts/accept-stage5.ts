...
async function run() {
  ...
  await check(12, 'Антиабуз', 'Разница уровней режет опыт', async () => {
    ...
    // ТЗ Этапа 5 просило «не более 10% при разнице 10». Действующая таблица
    // Этапа 1 даёт 30%, а ноль — при разнице от пятнадцати. Расхождение
    // названо прямо, а не спрятано подгонкой проверки.
    must(share <= 0.1 + 1e-9,
      `при разнице 10 остаётся ${Math.round(share * 100)}% опыта. Таблица Этапа 1 `
      + '(battleExp.levelDiffCoeff) даёт 0.3, ТЗ Этапа 5 просит не более 0.1. '
      + 'Одно из двух чисел лишнее — это вопрос баланса, а не дефект кода')
    return `при разнице 10 остаётся ${Math.round(share * 100)}%, при 15 — ноль`
  })
  ...
  pending.push(
    { id: 18, area: 'Баланс', title: 'Сквозной симулятор проходит все девять коридоров', why: 'три коридора ждут решения заказчика по вопросу В11' },
