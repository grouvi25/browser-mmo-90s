...
//
// Проверки 20–24 (релиз, документация) в этот прогон не входят — это шаги
// G6/G7. Они перечислены в отчёте как невыполненные, чтобы их нельзя было
// потерять. Проверки 18 (девять коридоров баланса) и 19 (антимастерство)
// раньше тоже были в этом списке — 18 сходилась только после пересчёта цен
// объектов и бара под В1·, 19 нашла настоящий баг двойного счёта в
// antiSkillLevel (04.09.2026) — обе теперь исполняются по-настоящему.
// =============================================================
...
async function run() {
  ...
  await check(18, 'Баланс', 'Сквозной симулятор проходит все девять коридоров', async () => {
    // Раньше в pending: три коридора не сходились. Ферма и окупаемость
    // объектов/бара пересчитаны 04.09.2026 (см. STAGE5_FULL_RUN_REPORT.md,
    // раздел про В1·) — теперь это настоящая исполняемая проверка, не
    // бумажная запись про открытый вопрос.
    const out = execFileSync('npx', ['tsx', '../scripts/simulate-full.ts', '--days', '30', '--seed', '4242'], {
      cwd: resolve(__dirname, '..'), encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'], shell: true,
    })
    const summary = out.split('\n').find(line => line.startsWith('ИТОГ'))
    must(summary?.includes('все коридоры сошлись'), summary ?? 'нет строки ИТОГ в выводе симулятора')
    return summary!
  })

  await check(19, 'Баланс', 'Антимастерство не делает специализацию проигрышной', async () => {
    // Тоже был в pending — сценарий из STAGE5_ACCEPTANCE.md раздел 2 ни разу
    // не прогонялся. Прогон 04.09.2026 нашёл настоящий баг (задвоение
    // antiSkillLevel в calcRawDamage — см. battle.formulas.ts) и настоящее
    // расхождение с ТЗ (потолок антимастерства был 10 вместо заявленных
    // «20/1–20/20»). Оба чинены в этом же коммите.
    const out = execFileSync('npx', ['tsx', '../scripts/simulate-antimastery.ts', '--duels', '3000', '--seed', '4242'], {
      cwd: resolve(__dirname, '..'), encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'], shell: true,
    })
    const summary = out.split('\n').find(line => line.startsWith('ИТОГ'))
    must(summary?.includes('не наказывает специализацию'), summary ?? 'нет строки ИТОГ в выводе симулятора')
    return summary!
  })
  ...
  pending.push(
    {
      id: 20, area: 'Релиз', title: 'Откат CD проверен фактическим откатом',
      why: 'проверено частично 04.09.2026: реальный старый образ (ebbc6355, тот самый коммит, что и правда часами крутился на проде после сбоя) запущен дриллом на изолированной копии базы, мигрированной вперёд до полной схемы, и остался здоровым — доказано, что старый код не падает на новой схеме, ровно то свойство, на которое опирается неполный откат CD. Сам bash-сценарий отката из cd.yml буквально не воспроизводился шаг за шагом — только это свойство',
    },
    {
      id: 21, area: 'Релиз', title: 'Полный прогон протоколов зелёный',
      why: 'accept:stage4 (22/22) и accept:stage5 (19/19) прогонялись многократно и по коду возврата — но через tsx из чекаута, не на собранном релизном докер-образе backend. Прод-эквивалентность (dist собирается, e2e-тесты CI гоняют dist/main.js) проверена отдельно, но не тем же прогоном, что и приёмка',
    },
    {
      id: 22, area: 'Релиз', title: 'Снимок базы с меткой релиза восстанавливается',
      why: 'реальный автоматический бэкап прода (mmo90s_20260903_212622.sql.gz) восстановлен в отдельную базу 04.09.2026 и мигрирован вперёд без ошибок — но это плановый бэкап, не снимок, специально помеченный как релизный',
    },
    {
      id: 23, area: 'Документация', title: 'Новый человек поднимает проект по инструкции',
      why: 'RELEASE_LAUNCH.md пройден буквально 04.09.2026 с нуля на чистом клоне — по пути нашлись и исправлены два реальных блокирующих бага (воркер падал на Windows, документ не упоминал экспорт .env в шелл). Прошёл я сам, знающий кодовую базу — настоящей проверки незнакомым человеком не было',
    },
    {
      id: 24, area: 'Документация', title: 'Четыре документа существуют и актуальны',
      why: 'все четыре (RELEASE_LAUNCH/ADMIN/SYSTEMS/LIMITATIONS.md) существуют, обновлены под каждую находку сессии 04.09.2026 (В13, ферма, окупаемость, антимастерство) — но не вычитаны никем со стороны, только автором правок',
    },
  )
