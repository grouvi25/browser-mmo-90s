...
export async function workRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{Params:{shiftId:string}}>('/shifts/:shiftId/claim',{preHandler:authenticate},async(req,reply)=>{if(!Id.safeParse(req.params.shiftId).success)return reply.code(422).send({code:'GEN_001',message:'Validation error'});const key=req.headers['idempotency-key'];if(typeof key!=='string')return reply.code(400).send({code:ErrorCode.ECON_IDEMPOTENCY_REQUIRED,message:'Idempotency-Key is required'});return reply.send(await WorkService.claim((await char(req.authUser.userId)).id,req.params.shiftId,key))})
