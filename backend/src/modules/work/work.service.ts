...
export const WorkService = {
  ...
  async start(characterId: string, productionObjectId: string) {
    if (!(await WorkRedis.tryConsumeDailyShift(characterId))) throw new AppError(ErrorCode.CONFLICT, 'Daily shift limit reached', 400)
    try {
      return await withTransaction(async tx => {
        ...
        if (!object?.isActive || object.status !== 'ACTIVE') throw new AppError(ErrorCode.CONFLICT, 'Production object unavailable', 400)
        if (character.productionLevel < object.requiredProductionLevel) throw new AppError(ErrorCode.CONFLICT, 'Production level too low', 400)
        if (character.status !== 'ACTIVE') throw new AppError(ErrorCode.CONFLICT, 'Character is busy', 400)
        if (await tx.workShift.findFirst({ where: { characterId, status: { in: ['ACTIVE','READY_TO_CLAIM'] } } })) throw new AppError(ErrorCode.CONFLICT, 'Active shift already exists', 409)
        const slots = await tx.workShift.count({ where: { productionObjectId, status: 'ACTIVE' } })
        if (slots >= object.workerSlots) throw new AppError(ErrorCode.CONFLICT, 'No free worker slots', 409)
        const claimed = await tx.character.updateMany({ where: { id: characterId, status: 'ACTIVE' }, data: { status: 'WORKING' } })
        if (claimed.count !== 1) throw new AppError(ErrorCode.CONFLICT, 'Character is busy', 409)
  ...
  async claim(characterId: string, shiftId: string, key: string) {
    return withIdempotency({ characterId, scope: 'work.shift.claim', key, execute: async tx => {
      ...
      if (shift.status !== 'READY_TO_CLAIM') throw new AppError(ErrorCode.CONFLICT, 'Shift is not ready', 400)
      ...
      if (changed.count !== 1) throw new AppError(ErrorCode.CONFLICT, 'Shift already claimed', 409)
