...
export async function privateShopsRoutes(fastify:FastifyInstance){
 ...
 fastify.post<{Params:{shopCode:string}}>('/:shopCode/buy',{preHandler:authenticate},async(req,reply)=>{const p=Buy.safeParse(req.body);if(!p.success)return reply.code(422).send({code:'GEN_001',message:'Validation error'});const key=req.headers['idempotency-key'];if(typeof key!=='string')return reply.code(400).send({code:ErrorCode.ECON_IDEMPOTENCY_REQUIRED,message:'Idempotency-Key is required'});const c=await CharactersRepository.findByUserId(req.authUser.userId);if(!c)throw new AppError(ErrorCode.CHARACTER_NOT_FOUND,'Character not found',404);return reply.send(await PrivateShopsService.buy(c.id,req.params.shopCode,p.data.privateShopItemId,p.data.quantity,key))})
}
