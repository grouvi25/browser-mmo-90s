...
const Query=z.object({page:z.coerce.number().int().min(1).default(1),limit:z.coerce.number().int().min(1).max(50).default(20),type:z.enum(['ITEM','RESOURCE']).optional(),mine:z.coerce.boolean().optional()})
export async function marketRoutes(fastify:FastifyInstance){const character=async(userId:string)=>{const c=await CharactersRepository.findByUserId(userId);if(!c)throw new AppError(ErrorCode.CHARACTER_NOT_FOUND,'Character not found',404);return c}
 ...
 fastify.post('/listings',{preHandler:authenticate},async(req,reply)=>{const p=Create.safeParse(req.body);if(!p.success)return reply.code(422).send({code:'GEN_001',message:'Validation error'});const key=req.headers['idempotency-key'];if(typeof key!=='string')return reply.code(400).send({code:'ECON_001',message:'Idempotency-Key is required'});const c=await character(req.authUser.userId);return reply.code(201).send(p.data.listingType==='ITEM'?await MarketService.createItem(c.id,p.data.itemInstanceId,p.data.price,key):await MarketService.createResource(c.id,p.data.resourceTemplateId,p.data.amount,p.data.price,key))})
 fastify.post<{Params:{listingId:string}}>('/listings/:listingId/buy',{preHandler:authenticate},async(req,reply)=>{const key=req.headers['idempotency-key'];if(typeof key!=='string')return reply.code(400).send({code:'ECON_001',message:'Idempotency-Key is required'});return reply.send(await MarketService.buy((await character(req.authUser.userId)).id,req.params.listingId,key))})
