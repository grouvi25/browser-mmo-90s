...
//
// Координаты (x, y) — столбец и ряд смещённой сетки. Величины взяты не
// из формулы, а сняты с решётки слоя «Клетки» файла «ПБ основа.psd»:
// поле там 10×10, ряды перспективные (клетки растут к переднему краю),
// и нечётные ряды сдвинуты ВЛЕВО на половину клетки. Поэтому соседство
// хранится таблицей (designer-grid-adjacency), а не считается по чётности:
// так логика хода не может разойтись с тем, что нарисовано.
//
// Расстояние — длина кратчайшего пути по этой же таблице.
// =============================================================
...
const TEAM_SPAWNS: Readonly<Record<1 | 2, readonly GridPosition[]>> = {
  // Якорь и его шесть соседей: отряд всегда встаёт связным кольцом.
  // Между якорями пять клеток — сходиться есть куда.
  1: [
    { x: 2, y: 5 }, { x: 1, y: 4 }, { x: 2, y: 4 },
    { x: 1, y: 5 }, { x: 3, y: 5 }, { x: 1, y: 6 },
    { x: 2, y: 6 }, { x: 0, y: 5 }, { x: 3, y: 4 },
  ],
  2: [
    { x: 7, y: 5 }, { x: 6, y: 4 }, { x: 7, y: 4 },
    { x: 6, y: 5 }, { x: 8, y: 5 }, { x: 6, y: 6 },
    { x: 7, y: 6 }, { x: 9, y: 5 }, { x: 8, y: 4 },
  ],
}
